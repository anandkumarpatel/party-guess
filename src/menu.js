...
class Menu extends Component {
  ...
  toggleBtn = (cat) => {
    ...
    newBtnState.forEach((x, i) => (i !== currentBtn ? (x.isActive = false) : (x.isActive = !x.isActive)))
              ...
              >
                <div className='Menu-side Menu-side--back'>
                  ...
                  <div className='Menu-side-btnWrap'>
                    {cat.isLocked === 'TRUE' ? (
                      <Link className='Menu-side-btn Menu-side-btn--go' to={`/login/${this.getCategoryURL(cat.name)}`}>
                ...
                >
                  {cat.name}
                  {cat.isLocked === 'TRUE' && <div className='Menu-side-saleFlag'>$0.99!</div>}
                  {cat.name.toLowerCase().includes('kid') && <div className='Menu-side-saleFlag'>Kids!</div>}
                  <p className='Menu-info'>{`${cat.list.size} cards left`}</p>
                </div>
              </div>
            ))}
        </div>
